...
public class PixImage {
      ...
      for (int i = 0; i < w; i++) {
        for (int j = 0; j < h; j++) {
          if ((i == 0 || i == w-1) && (j == 0 || j == h-1)) {
            int[] avg = sumRGB(i, j, curr);
            copy.setPixel(i, j, (short)(avg[0]/4), (short)(avg[1]/4), 
                                (short)(avg[2]/4));
            //corner
          }
            ...
            (j < h - 1) && (i == 0 || i == w - 1)) {
            int[] avg = sumRGB(i, j, curr);
            copy.setPixel(i, j, (short)(avg[0]/6), (short)(avg[1]/6), 
                                (short)(avg[2]/6));
            //edge
          } else {
          int[] avg = sumRGB(i, j, curr);
          copy.setPixel(i, j, (short)(avg[0]/9), (short)(avg[1]/9), 
                              (short)(avg[2]/9));
          }
        }
      }
  ...
  public int[] sumRGB(int x, int y, PixImage curr) {
    int[] sum = new int[3];
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        if (inside(x+i, y+j)) {
        sum[0] += curr.getRed(x+i, y+j);
        sum[1] += curr.getGreen(x+i, y+j);
        sum[2] += curr.getBlue(x+i, y+j);
        }
      }
    }
    return sum;
