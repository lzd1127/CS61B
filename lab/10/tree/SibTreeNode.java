...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
    ...
    if (!isValidNode()) {
      throw new InvalidNodeException("'this' node is invalid");
    }
    if (myTree.root() == this) {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (c <= 1) {
      ...
      SibTreeNode prevNode = (SibTreeNode) this.child(c-1);
      if (!prevNode.isValidNode()) {
        prevNode = firstChild;
        while (prevNode.nextSibling != null) {
          prevNode = prevNode.nextSibling;
        }
      }
      newNode.nextSibling = prevNode.nextSibling;
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (myTree.root == this) {
      ...
      SibTreeNode check = parent.firstChild;
      if (check == this) {
        parent.firstChild = this.nextSibling;
        valid = false;
      } else {
        while (check.nextSibling != this) {
          check = check.nextSibling;
        }
        check.nextSibling = nextSibling;
        valid = false;
      }
    }
    myTree.size--;
