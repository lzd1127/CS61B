...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    DisjointSets newMaze = new DisjointSets(horiz*vert);
    int hW = horiz * (vert-1);
    int vW = (horiz-1) * vert;
    int[] newWalls = new int[hW + vW];
    ...
    for (i = hW + vW; i > 0; i--) {
      int random = randInt(i);
      int swap = newWalls[i-1];
      newWalls[i-1] = newWalls[random];
      newWalls[random] = swap;
    }
    ...
    for (i = 0; i < hW + vW; i++) {
      ...
      if (w > 0) {
        ...
        int belowCell = belowX + belowY*horiz;

        if (newMaze.find(topCell) != newMaze.find(belowCell)) {
          hWalls[topX][topY] = false;
          newMaze.union(newMaze.find(topCell), newMaze.find(belowCell));
        }
        ...
        if (newMaze.find(leftCell)!=newMaze.find(rightCell)) {
          vWalls[leftX][leftY] = false;
          newMaze.union(newMaze.find(leftCell), newMaze.find(rightCell));
        }
      }
    }
  }
